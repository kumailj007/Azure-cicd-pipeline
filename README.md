...
Backs my **AZ-400 (DevOps Engineer Expert)** certification. The CI stages run automatically on GitHub on every push — the green badge above is live proof the automation works. The Azure deploy stage is defined and ready, gated behind a repo variable so the pipeline stays green without a subscription.
...
| Release strategy       | Trunk-based branching, SHA + semver image tags ([BRANCHING-STRATEGY.md](BRANCHING-STRATEGY.md)) |
| Secure pipelines       | Secrets via GitHub Secrets, deploy gated behind an explicit repo variable   |
